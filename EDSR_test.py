...
def test(activate,activate_name):
    print('start Activate:{} Test'.format(activate_name))
    edsr = EDSR(act=activate)

    print('loading weight...')
    edsr.load_state_dict(torch.load(os.path.join(root,'model','edsr_2_{}_params.pt'.format(activate_name))))
    ...
    paths = glob.glob(os.path.join(root,'dataset_test','*.png'))
    ...
    for i,datas_path in enumerate(paths):
        ...
        loss_list.append(loss.cpu().data[0])
        psnr_list.append(psnr)
        ssim_list.append(ssim)
        
        print('image:{} loss:{} psnr:{} ssim:{}'.format(i,loss.cpu().data[0],psnr,ssim))

    avg_loss = avg_loss/len(paths)
    avg_psnr = avg_psnr/len(paths)
    avg_ssim = avg_ssim/len(paths)
    print('\navg loss:{}, avg psnr:{}, avg ssim:{}\n'.format(avg_loss,avg_psnr,avg_ssim))

    with open('test_status\\'+'test_loss_{}.txt'.format(activate_name),'w') as f:
        for i,loss in enumerate(loss_list):
            if i==len(loss_list)-1:
                f.write(str(loss))
            else:
                f.write(str(loss)+',')
    axis = np.linspace(0, len(paths)-1, len(paths))
    fig = plt.figure()
    plt.xlabel('image id')
    plt.ylabel('loss')
    plt.title('test activate:{}'.format(activate_name))
    plt.grid(True)
    plt.plot(axis, loss_list)
    plt.legend()
    plt.savefig('test_status\\'+'test_loss_{}.pdf'.format(activate_name))
    plt.close(fig)

    with open('test_status\\'+'test_psnr_{}.txt'.format(activate_name),'w') as f:
    ...
    plt.legend()
    plt.savefig('test_status\\'+'test_psnr_{}.pdf'.format(activate_name))
    plt.close(fig)

    with open('test_status\\'+'test_ssim_{}.txt'.format(activate_name),'w') as f:
    ...
    plt.legend()
    plt.savefig('test_status\\'+'test_ssim_{}.pdf'.format(activate_name))
    plt.close(fig)