...
def train(epoch,activate,activate_name):
    edsr = EDSR(act=activate)
    print('start AllEpoch:{} Activate:{} Train'.format(epoch,activate_name))
    #重载入模型
    if if_restore:
        print('loading weight...')
        edsr.load_state_dict(torch.load(os.path.join(root,'model','{}_model_lastest.pt'.format(activate_name))))
    ...
    for step in range(epoch):
        ...
        with open('train_status\\'+'train_loss_{}.txt'.format(activate_name),'w') as f:
        ...
        plt.legend()
        plt.savefig('train_status\\'+'train_loss_{}.pdf'.format(activate_name))
        plt.close(fig)
        
        with open('train_status\\'+'train_psnr_{}.txt'.format(activate_name),'w') as f:
        ...
        plt.legend()
        plt.savefig('train_status\\'+'train_psnr_{}.pdf'.format(activate_name))
        plt.close(fig)
        
        with open('train_status\\'+'train_ssim_{}.txt'.format(activate_name),'w') as f:
        ...
        plt.legend()
        plt.savefig('train_status\\'+'train_ssim_{}.pdf'.format(activate_name))
        plt.close(fig)
        
        if step%99==0:
            if step%2==0:
                torch.save(edsr,'model\\edsr_2_{}.pt'.format(activate_name))
                torch.save(edsr.state_dict(),'model\\edsr_2_{}_params.pt'.format(activate_name))
            else:
                torch.save(edsr,'model\\edsr_1_{}.pt'.format(activate_name))
                torch.save(edsr.state_dict(),'model\\edsr_1_{}_params.pt'.format(activate_name))
