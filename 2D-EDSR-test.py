...
activate = activate_list['4'][0]
activate_name = activate_list['4'][1]
####################
edsr = EDSR(act=activate)
print(edsr)

print('loading weight...')
edsr.load_state_dict(torch.load(os.path.join(root,'model','edsr_1_params.pt')))
...
with open('test_loss_{}.txt'.format(activate_name),'w') as f:
...
plt.legend()
plt.savefig('test_loss_{}.pdf'.format(activate_name))
plt.close(fig)

with open('test_psnr_{}.txt'.format(activate_name),'w') as f:
...
plt.legend()
plt.savefig('test_psnr_{}.pdf'.format(activate_name))
plt.close(fig)

with open('test_ssim_{}.txt'.format(activate_name),'w') as f:
...
plt.legend()
plt.savefig('test_ssim_{}.pdf'.format(activate_name))
plt.close(fig)